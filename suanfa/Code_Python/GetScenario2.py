...
class Scenario2:
    ...
    def __init__(self,PRC,NRC,Send_inf,Rec_inf,scale,S_name,Send_Sen,Rec_Sen,S_0,S_l,S_u,Hy,Ps,If_coup,Coup_S_list1,Coup_S_list1_No,Coup_S_list2,Coup_S_list2_No,Send_HeN,Zr) -> None:
        """
        self.PRC = PRC# 前面放水电、直流落点片区后面放参与消纳的片区
        self.NRC = NRC# 前面放水电、直流落点片区后面放参与消纳的片区
        self.Send_inf = Send_inf#水电、直流所在的片区
        self.Rec_inf = Rec_inf#参与消纳的片区
        self.scale = scale#表示场景：1水电消纳；2直流消纳
        self.S_name = S_name
        self.Send_Sen = Send_Sen#水电、直流所在的片区对断面的灵敏度
        self.Rec_Sen = Rec_Sen#参与消纳的片区所在的片区对断面的灵敏度
        """
        self.PRC = PRC# 前面放水电、直流落点片区后面放参与消纳的片区
        self.NRC = NRC# 前面放水电、直流落点片区后面放参与消纳的片区
        self.Send_inf = Send_inf#水电、直流所在的片区
    ...
    def Get_a(self):
        ...
        Length_F = len(self.PRC)-Length-1*len(self.Send_inf)
        try:
            ...
            self.a = sc.linalg.block_diag(np.eye(len(self.Send_inf)),self.a)
        except:
    ...
    def Cal_S(self):
        # if self.scenario in [1,2,3]:
        tmp_S1 = np.hstack((self.Send_Sen,self.Rec_Sen))
        self.cal_S2 = np.dot(tmp_S1,self.a)
        pass
    
    def Solve(self):
        
        plant_num = self.cal_S2.shape[1]
        ...
        if self.scale == 1:
            ...
            name = '直流消纳DCA'
        Scenario_Solve = pl.LpProblem(name,sense=pl.LpMaximize)
        # 定义辅助变量
        ...
        for i in range(0,len(self.Send_inf)):
            tmp_1[0,i] = 1
        ...
        if self.Send_HeN == 1:# 只包括河南全部片区
            tmp_a = np.dot(tmp_2-tmp_1,self.a)
            Scenario_Solve += (pl.lpSum([tmp_a[0,j]*(self.PRC[j]-Var_delta_P[j]) for j in range(len(self.Send_inf),plant_num)]))>=self.Zr[1]+0.2*self.Zr[2]     
        elif self.Send_HeN  == 2:# 全网片区
            tmp_a = np.dot(tmp_2-tmp_1,self.a)
            Scenario_Solve += (pl.lpSum([tmp_a[0,j]*(self.PRC[j]-Var_delta_P[j]) for j in range(len(self.Send_inf),19+len(self.Send_inf))]))>=self.Zr[1]+0*self.Zr[2]
            ...
            Scenario_Solve += (pl.lpSum([(self.PRC[j]-Var_delta_P[j]) for j in range(len(self.Send_inf),plant_num)]))>=0*self.Zr[1]+0.1*self.Zr[2]
