...
class Scenario1:
    ...
    coup_dict2 = {'EY-EX': np.array([[3000, 0, -1000, -3000, -5000, -5600], [-2500, -1500, -1000, -500, -0, 500]]),
                  'EY-EG': np.array(
                      [[2300, 400, -1300, -2200, -3400, -5000, -5600], [-1800, -1500, -1200, -1000, -700, -0, 500]]),
                  'YZYB-MS': [[-1500, 1500], [-3300, -2800]],
                  'EX-XG': 3200,
                  'EG-XG': 3200}

    def __init__(self, scenario, PRC, NRC, Send_inf, Rec_inf, scale, S_name, Send_Sen, Rec_Sen, S_0, S_l, S_u, Hy, Ps,
                 If_coup, Coup_S_list1, Coup_S_list1_No, Coup_S_list2, Coup_S_list2_No, Send_HeN, Zr) -> None:
    ...
    def Solve(self):
        ...
        # 定义片区功率调整量
        Var_delta_P = [
            pl.LpVariable('P' + str(i), lowBound=-plant_lowbounds[i], upBound=plant_upbounds[i], cat=pl.LpContinuous)
        ...
        for i in range(0, section_num):
            Scenario_Solve += (pl.lpSum([Sentivity[Z[i]][j] * Var_delta_P[j] for j in range(0, plant_num)]) <= Con1[
                Z[i] + '上裕度'])
            # Scenario_Solve += (pl.lpDot(Sentivity['断面'+str(i)],Var_Z)<=Con1['断面'+str(i)])
        for i in range(0, section_num):
            Scenario_Solve += (pl.lpSum([Sentivity[Z[i]][j] * Var_delta_P[j] for j in range(0, plant_num)]) >= Con2[
                Z[i] + '下裕度'])
        ...
        if self.If_coup == 1:
            ...
            if Length1 > 0:
                ...
                for jj in range(0, Length1):
                    ...
                    if '豫中-豫北' not in Z[self.Coup_S_list1_No[0, jj]]:
                        Scenario_Solve += self.Coup_S_list1[0, jj] - (pl.lpSum(
                            [Sentivity[Z[-1 + self.Coup_S_list1_No[0, jj]]][j] * Var_delta_P[j] for j in
                             range(0, plant_num)])) - 1000000 + 1000000 * Var_bivar[jj] <= 0
                        Scenario_Solve += self.Coup_S_list1[0, jj] - (pl.lpSum(
                            [Sentivity[Z[-1 + self.Coup_S_list1_No[0, jj]]][j] * Var_delta_P[j] for j in
                             range(0, plant_num)])) - 0.01 + 1000000 * Var_bivar[jj] >= 0
                        Scenario_Solve += self.Coup_S_list1[1, jj] - (pl.lpSum(
                            [Sentivity[Z[-1 + self.Coup_S_list1_No[1, jj]]][j] * Var_delta_P[j] for j in
                             range(0, plant_num)])) - 1000000 + 1000000 * Var_bivar[jj] <= 0
            ...
            for jj in range(0, Length2):
                ...
                Scenario_Solve += self.Coup_S_list2[0, jj] * section_moment[-1 + self.Coup_S_list2_No[0, jj]] + \
                                  self.Coup_S_list2[1, jj] * section_moment[-1 + self.Coup_S_list2_No[1, jj]] + \
                                  self.Coup_S_list2[0, jj] * (pl.lpSum(
                    [Sentivity[Z[-1 + self.Coup_S_list2_No[0, jj]]][j] * Var_delta_P[j] for j in
                     range(0, plant_num)])) + \
                                  self.Coup_S_list2[1, jj] * (pl.lpSum(
                    [Sentivity[Z[-1 + self.Coup_S_list2_No[1, jj]]][j] * Var_delta_P[j] for j in
                     range(0, plant_num)])) <= 3200

                # 备用容量约束
        if self.Send_HeN == 1:  # 只包括河南全部片区
            tmp_a = np.dot(np.ones((1, plant_num - len(self.Hy) - len(self.Ps))), self.a)
            Scenario_Solve += (pl.lpSum([tmp_a[0, j] * (self.PRC[j] - Var_delta_P[j]) for j in range(0, plant_num)])) >= \
                              self.Zr[1] + 0.2 * self.Zr[2]
        elif self.Send_HeN == 2:  # 全网片区
            tmp_a = np.dot(np.ones((1, plant_num - len(self.Hy) - len(self.Ps))), self.a)
            Scenario_Solve += (pl.lpSum([tmp_a[0, j] * (self.PRC[j] - Var_delta_P[j]) for j in range(0, 19)])) >= \
                              self.Zr[1] + 0 * self.Zr[2]
            Scenario_Solve += (pl.lpSum([(self.PRC[j] - Var_delta_P[j]) for j in range(0, plant_num)])) >= self.Zr[
                1] + 0.2 * self.Zr[2]
        elif self.Send_HeN == 3:  # 不包括河南省网的情况
            Scenario_Solve += (pl.lpSum([(self.PRC[j] - Var_delta_P[j]) for j in range(0, plant_num)])) >= 0 * self.Zr[
                1] + 0.2 * self.Zr[2]
        ...
        # Scenario_Solve.solve(solver=pl.get_solver(pl.listSolvers(onlyAvailable=True)[0]))
